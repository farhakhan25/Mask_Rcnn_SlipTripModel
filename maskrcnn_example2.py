...
from myutils import extract_index_nparray, get_bounding_box, detect_landmarks, get_delauny_triangles, get_delauny_triangles, \
...
# Load image
fpath = ROOT_DIR + f"/Input_Video/video/slip2.mp4"
...
fpath = ROOT_DIR + f"/results2/slip_prediction_2.avi"
...
while True:
    if f<50:
        ...
        for i in range(detection_count_ini):
            ...
            img_ini, mask0_ini, roi_ini, roi_gray_ini, roi_b_ini, roi_mask_ini = get_bounding_box(i, box_ini, img_ini, black_image_ini, masks_ini, width_ini, height_ini, class_id_ini)
            mask_ini, roi_ini, landmarks_points_ini = detect_landmarks(mask0_ini, roi_b_ini)
            roi_ini, roi_mask_ini, image_1_ini, indexes_triangles_ini, triangle_ini, centriod_ini = get_delauny_triangles(landmarks_points_ini, roi_ini, roi_mask_ini)
            ...
            # HOG
            fd_ini, hog_image_ini = hog(image_1_ini, orientations=2, pixels_per_cell=(4, 4), cells_per_block=(1, 1), visualize=True)
            ho_image_rescaled_ini = exposure.rescale_intensity(hog_image_ini, in_range=(0, 0.02))
            result_hog.append(np.mean(fd_ini))

            # Brute Force
            contours_ini, _ = cv2.findContours(np.array(mask0_ini, np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            height_ini, width_ini, _ = roi_ini_n.shape
            spacing = 5
            scatter = 0.5
            vertices_ini = scatter_vertices(plane_width=width_ini, plane_height=height_ini, spacing=spacing, scatter=scatter)
            ...
            all_points_ini, key_points_all_ini, centroids_all_ini, centroids_all_kp_ini, roi_ini_n = extract_contour_points(triangles_ini, contours_ini[max_len_cont], landmarks=landmarks_points_ini, triangle_type="all",roi=roi_ini_n)
        ...
        for i in range(detection_count_fin):
            ...
            # Brute Force
            contours_fin, _ = cv2.findContours(np.array(mask0_fin, np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            height_fin, width_fin, _ = roi_fin_n.shape
            vertices_fin = scatter_vertices(plane_width=width_fin, plane_height=height_fin, spacing=spacing, scatter=scatter)
            triangles_fin = delaunay(vertices=vertices_fin,delete_super_shared=False)
            cnt_len = [len(c) for c in contours_fin]
            max_len_cont = cnt_len.index(max(cnt_len))
            all_points_fin, key_points_all_fin, centroids_all_fin, centroids_all_kp_fin, roi_fin_n = extract_contour_points(triangles_fin, contours_fin[max_len_cont], landmarks=landmarks_points_fin, triangle_type="all", roi=roi_fin_n)

            #kp_fin, des_fin = orb.compute(roi_fin_n, key_points_all_fin)
            ...
            matching_result_flann = cv2.drawMatchesKnn(roi_ini_n, kp_sift_ini, roi_fin_n, kp_sift_fin, matches, None, **draw_params)
            avg_dis = np.mean(good_matches_d)

            ## HOG
            fd_fin, hog_image_fin = hog(image_1_fin, orientations=2, pixels_per_cell=(4, 4), cells_per_block=(1, 1), visualize=True)
            ...
            if not len(result_avgdis) ==0 and not len(result_hog) ==0:
                print(np.mean(result_avgdis)-np.std(result_avgdis), np.mean(result_avgdis), np.mean(result_avgdis)+np.std(result_avgdis), np.mean(result_avgdis)+2*np.std(result_avgdis), np.mean(result_avgdis)+3*np.std(result_avgdis))
                print(np.mean(fd_fin), np.mean(result_hog) - np.std(result_hog), np.mean(result_hog), np.mean(result_hog) + np.std(result_hog), np.mean(result_hog) + 2*np.std(result_hog), np.mean(result_hog) + 3*np.std(result_hog))
                if (avg_dis > 210 and (np.mean(fd_fin) < 0.144 or (hog_amd > 0.01))) or hog_amd > 0.01 or avg_dis > 223 or np.mean(fd_fin) < 0.144:
                   title = "slip"
            ...
            df = pd.DataFrame(result_parameter, columns=['frame_id', "avg_brut_dist", "hog_mean_prev", "hog_mean_curnt", "hog_abs_mean_diff","class_label"])
            fpath = ROOT_DIR + f"/results2/slip_result_2.csv"
        ...
        cv2.imwrite(os.path.join(ROOT_DIR +'/results2/ori/' , f'frame{f}.jpg'), img_fin_ori)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/gray/', f'frame{f}.jpg'), img_fin_ori_gray)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/box/', f'frame{f}.jpg'), ori_image_fin)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/mask/', f'frame{f}.jpg'), image_1_fin)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/triangles/', f'frame{f}.jpg'), roi_fin_n)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/flann/', f'frame{f}.jpg'), matching_result_flann)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/hog/', f'frame{f}.jpg'), hog_image_fin)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/final/', f'frame{f}.jpg'), img_fin)
        cv2.imwrite(os.path.join(ROOT_DIR + '/results2/class/', f'frame{f}.jpg'), ori_image_class)
...
plt.draw()
plt.savefig("abs_hog_diff_1.png")
plt.show()
plt.pause(0.01)
...
plt.draw()
plt.savefig("avg_brute_dist_1.png")
plt.show()
plt.pause(0.01)